...
为提高调试效率和任务编排的灵活性，我们用 Python 编写带有 GUI 的上位机程序。上位机程序的核心是任务调度引擎，其主要作用是将复杂任务拆分为多个可复用的原子动作，并按照预设逻辑进行组织和执行。

我们预先定义一组基础动作，例如移动、夹取、识别、等待、状态判断等。任务调度引擎根据任务需求，将这些原子动作组合成完整的执行流程。通过这种方式，可以减少重复代码编写，提高任务开发效率，同时也便于在调试阶段快速修改动作顺序、调整参数或替换单个功能模块。

此外，上位机还负责视觉数据处理和运行状态反馈。视觉运算服务接收相机图像数据，对目标进行识别和跟踪，并将识别结果传递给任务调度模块。通信中继服务则负责将上位机生成的控制指令发送至下位机，同时接收下位机返回的运行状态和传感器数据，用于辅助任务决策和状态显示。

=== 下位机程序架构

由于下位机调试相对不便，因此下位机不直接承担复杂任务规划，而是循环接收上位机发送的控制指令，对指令进行解析，并执行对应的底层动作。

下位机程序的核心是指令解析器。系统在上位机中预先定义统一的指令格式和指令集，下位机只需根据指令内容调用相应的控制模块即可。例如，当接收到底盘移动指令时，下位机会进行底盘运动学，并结合编码器、陀螺仪等传感器数据进行 PID 闭环控制；当接收到夹取或机构调整指令时，则解算机构位置，并通过 PWM 输出控制相应执行机构。

这种设计将复杂逻辑集中在便于调试的上位机中，而下位机仅负责稳定、可靠地执行标准化指令，降低了下位机程序的复杂度，也提高了系统整体的可靠性。通过统一的通信协议和模块化的控制结构，后续如需增加新的动作或执行机构，只需扩展指令集和对应的执行模块即可。
